...
(define (draw-game state)
  (cond
    ...
    [(equal? "pause" (appstate-game state)) (overlay 'center 'center GAME-OVER (draw-appstate state))]
...
(define (move state)
  (cond
    ...
    [(equal? "string" (appstate-game state)) 0]
...
; Examples
(check-expect (pause DEFAULT) (make-appstate
                               (appstate-snake DEFAULT)
                               (appstate-apple DEFAULT)
                               #false
                               (appstate-quit DEFAULT)))
(check-expect (pause (make-appstate
                               (make-snake (list (make-posn 463 13) (make-posn 488 13) (make-posn 513 13)) 3 RIGHT)
                               (appstate-apple DEFAULT)
                               #true
                               (appstate-quit DEFAULT)))
              (make-appstate
...
(define (pause state)
  (cond
    [(or
      (check-position-out (last (snake-position (appstate-snake state))) BACKGROUNDPOS) ; if the snake is over background's limits, the application turn off
      (check-eat-snake (appstate-snake state)))                                          ; if the snake hits itself, the application turn off
     (make-appstate
      (appstate-snake state)
      (appstate-apple state)
      "pause"
