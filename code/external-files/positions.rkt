...
(provide increment-pos)
(provide decrement-pos)
(provide check-position-out)
(provide compute-available-pos)
(provide update-positions)
(provide cut-breakpoints)
(provide direction-by-posn)
(provide compute-apple-position)
    ...
    [(and (equal? (posn-y pos) (posn-y (first lob))) (not (posn-after? last d pos (first lob))))
     (cond
       ...
       [else
        pos])]
    [else
     (posn-dir-breakpoints last d pos (rest lob))]))
...
(check-expect (cut-breakpoints 3 1 '()) '())
